...
- CSS Modules を変更したら `pnpm run tcm` で型定義を再生成すること。ビルドが型エラーで落ちる原因の多くはこれ
- 記事データは `content/posts/*.md` → Velite → `.velite/` にビルドされる。型は `.velite/` から import（`import type { Post } from "@/.velite"`）
- `noUncheckedIndexedAccess: true` のため、配列・オブジェクトへの index 経由アクセスは `undefined` を含む型になる。適切にガードすること
- `enum` は禁止（`erasableSyntaxOnly: true`）— union type を使用
- `interface` ではなく `type` を使用（ESLint `consistent-type-definitions: "type"`）
- `className` に template literal 禁止 — `clsx()` を使用
- 非 null アサーション（`!`）禁止（ESLint `no-non-null-assertion`）
- オブジェクト・インターフェースのキーはアルファベット順（perfectionist プラグインで強制）
...
UI を実装するときは必ず @DESIGN.md を参照してスタイルを適用すること。

## Rules

詳細なコーディング規約は `.claude/rules/` を参照:
- @.claude/rules/react-typescript.md — React/TypeScript 規約
- @.claude/rules/css-modules.md — CSS Modules 規約
- @.claude/rules/design-system.md — デザインシステム（Liquid Glass）
- @.claude/rules/content.md — コンテンツ管理（Velite フロントマター）
