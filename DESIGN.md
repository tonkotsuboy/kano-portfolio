...
### Do
- 背景アンビエントメッシュを必ず敷く
- カードには **blur + saturate + 内側ハイライト** の 3 点セットを守る
...
**Files**: `app/styles/globals.css`（実トークン実装）/ `.claude/rules/design-system.md`（運用規約）

---

## Integration

kano-portfolio 本体リポジトリの `CLAUDE.md` に以下を追記してください：
...
これで Claude Code / Cursor 等のエージェントが UI タスクを受け取ったとき、本ファイルを自動で読み込み、色・型・コンポーネント仕様を遵守した実装を出力します。
